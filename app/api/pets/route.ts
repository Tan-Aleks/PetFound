...
export async function GET(request: Request) {
  try {
    ...
    const validTypes = ['small', 'dog', 'cat', ''] as const
    const validStatuses = ['lost', 'found', ''] as const
    ...
    if (
      ...
      query = query.eq('type', type as 'small' | 'dog' | 'cat')
    }
    ...
    if (
      ...
      query = query.eq('status', status as 'lost' | 'found')
    }
