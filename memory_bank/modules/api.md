...
- `GET /api/ai-match-chat/conversations`: список системных AI-диалогов по найденным внешним совпадениям.
- `GET /api/ai-match-chat/[matchId]`: загрузка AI-чата и контекста конкретного совпадения.
- `POST /api/ai-match-chat/[matchId]/message`: задать вопрос AI по совпадению в рамках разрешенных тем.
...
- `POST /api/messages` после успешной записи сообщения дополнительно создает `notifications.message_received` для получателя, чтобы Header и dropdown сразу отражали новый диалоговый event.
- AI-чат по совпадениям не использует таблицу `messages`; он опирается на `ai_match_messages`, чтобы системный помощник оставался отдельным защищенным контуром.
