...
- Для AI-обсуждения внешних совпадений используется отдельная таблица `ai_match_messages`; это исключает смешивание системного помощника с обычными сообщениями людей в `messages`.
- AI-чат работает только по фактам одного `cross_matches` и серверно ограничивает темы ответов: сайт, ссылка, локация, контакты и сведения о питомце.
